# Research Artifact: Slot Machine Mechanics and Psychology
**Researcher:** Han Yang-Lin

## Slot Machine Terminology

- **Reel:** The object that symbols are placed on, and starts spinning when the player plays the machine.
- **Payline:** A line that crosses through one symbol per reel, on which winning combinations are evaluated. The simplest slot machine has a payline running horizontally straight across the middle. The number of paylines can range from one to thousands, and the player can decide the amount to wager on each payline.
- **Return To Player:** The theoretical payout percentage as winnings as a percentage of the money wagered by the players. The minimum payout percentage is usually regulated by law (e.g. 75% in Nevada).
