...
const REACTIVE_STYLE_ELEMENT_ID = "iteration-14-reactive-styles";
...
function ensureReactiveStateStyles() {
  ...
  styleElement.textContent = `
    ...
    .symbol-slot.${MATCHED_SLOT_CLASS_NAME} {
      box-shadow: inset 0 0 0 3px rgb(255 248 234 / 65%);
...
function createAnnouncementMessage(statusMessage, payoutAmount, matchedPaylineCount, scatterCount) {
  ...
  if (payoutAmount > 0) {
    return `${statusMessage} ${matchedPaylineCount} payline${matchedPaylineCount === 1 ? "" : "s"} matched.`;
  }

  if (scatterCount > 0) {
    return `${statusMessage} ${scatterCount} Scatter symbol${scatterCount === 1 ? "" : "s"} landed.`;
  }
