...
const REACTIVE_STYLE_ELEMENT_ID = "iteration-19-reactive-styles";
...
const WIN_AMOUNT_OVERLAY_ID = "iteration-19-win-amount-overlay";
const COIN_SHOWER_LAYER_ID = "iteration-19-coin-shower-layer";
...
function handleSpin(dom) {
  ...
  void performSpinWithAnimation(dom, activeWager);
}
 ...
 */
async function performSpinWithAnimation(dom, activeWager) {
...
function createReactiveVisualState(gameState) {
  ...
  const shouldAnimateWinFeedback =
    payoutAmount > 0 && gameState.statusMessage.startsWith("Spent ") && !prefersReducedMotion();
...
function renderWinFeedback(dom, reactiveVisualState) {
  if (!reactiveVisualState.shouldAnimateWinFeedback) {
    clearWinFeedback(dom);
    return;
  }
